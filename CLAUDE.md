...
| M7 | **"demain"** | Autorisé dans `boost_preparing` et les bandeaux post-complétion boost (hero vert, archive boost). Interdit dans les messages chapitre post-complétion, toasts et coach tips |
...
| G7 | **Slots de 5** | Chapitres ≥20 exos découpés en 4 slots visuels (5/10/15/20). Overlay récompense +75 XP aux paliers 5/10/15 + **ligne comparaison** "X% vs Y% précédent" si historique existe. Palier 20 absorbé par `chkComp`. Chapitres <20 exos : pas de slots, +300 XP classique. Count-based (pas index-based) — fire au 5ème exo fait, quel que soit l'ordre. Absorbe `_checkCoursMilestone` quand un slot fire |
...
| G9 | **Sessions retro avec %** | Chapitres et boosts terminés : pills par **date** (pas "Passage N"). Score % coloré sur chaque pill (vert ≥70%, ambre ≥40%, rose <40%) + flèche `↑/↓/→` vs session précédente. **Carte fermée terminée** : derniers 3 passages avec date + % + flèche (remplace slots/barre). **Carte fermée en cours + historique** : slots/barre + comparaison inline "X% ↑ vs Y%". Exercices retro affichés en read-only avec **barre de numéros cliquable**. Système de pills unifié pour chapitres et boosts. Onglet Progression masqué — toute l'info est sur les cartes |
