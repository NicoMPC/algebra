...
- **Tri chapitres dashboard** : BOOST (fixe en tête) → entamés (exos faits hors calibrage, pas terminés) → révision (cross-niveau) → pas commencés → terminés. Basé sur `S.chapTouched` (exclut `source=CALIBRAGE` et `source=BOOST`). Flèches tendance via `S.chapSessions` (même filtrage — pas de session fantôme sur chapitres seulement diagnostiqués)
