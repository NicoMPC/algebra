...
async function register(p: Record<string, unknown>) {
  ...
  // Audit 2026-04-11 P0 : whitelist chars pour bloquer XSS stored dans le dashboard admin.
...
async function saveScore(p: Record<string, unknown>) {
  ...
  // Insert with ON CONFLICT dedup
  // FIX 2026-04-11 P0 : l'index unique est (code,chapitre,num_exo,date,source).
  // Avant ce fix le onConflict listait 4 cols → PostgREST 42P10 sur tous les doublons
  // → save_scores_batch/saveScore échouaient silencieusement. Scores perdus 9 jours.
  ...
  }, { onConflict: "code,chapitre,num_exo,date,source", ignoreDuplicates: true });
  if (insertErr) {
...
async function updateConfidenceScore(
  ...
  // FIX 2026-04-11 P0 : l'ancien code lisait/écrivait `progress.nb_easy` qui N'EXISTE
  // PAS dans le schéma (seulement nb_exos, nb_erreurs, score) → toute écriture progress
  // échouait silencieusement (PGRST204). Refactor : on recompte depuis `scores` à la
  // volée (source de vérité). 1 query supplémentaire mais correct et sans DDL.
  ...
  const relevant = (scoreRows || []).filter((s: Record<string, unknown>) => {
    ...
    return src !== "BOOST" && src !== "CALIBRAGE";
  });
...
async function saveCalibrationBatch(p: Record<string, unknown>) {
  ...
  // Refonte 10/04/2026 : scoring pondéré par importance Brevet + tie-break explicite
...
async function saveScoresBatch(p: Record<string, unknown>) {
  ...
  // FIX 2026-04-11 P0 : onConflict doit matcher l'index unique (5 cols, pas 4).
  ...
  // FIX 2026-04-11 P0 : ne plus utiliser `progress.nb_easy` (colonne inexistante).
...
async function publishAdminBoost(p: Record<string, unknown>) {
  // Audit 2026-04-11 P0 : exiger is_admin sur le caller (défense en profondeur).
  // Note : le front envoie adminCode séparé de targetCode. Backward-compat si adminCode absent → fallback code.
  const adminCode = String(p.adminCode || p.code || "");
  const { data: admin } = await adminClient.from("profiles")
    .select("is_admin").eq("code", adminCode).maybeSingle();
  if (!admin?.is_admin) return { status: "error", message: "Accès refusé." };
...
async function publishAdminChapter(p: Record<string, unknown>) {
  // Audit 2026-04-11 P0 : exiger is_admin sur le caller.
  const adminCode = String(p.adminCode || p.code || "");
  const { data: admin } = await adminClient.from("profiles")
    .select("is_admin").eq("code", adminCode).maybeSingle();
  if (!admin?.is_admin) return { status: "error", message: "Accès refusé." };
...
async function resetPassword(p: Record<string, unknown>) {
  ...
  // ⚠ Audit 2026-04-11 : flow custom token cassé (aucun token stocké côté back).
  // Refus systématique — le vrai reset passe par le lien Supabase Auth envoyé par forgot_password.
  // L'élève doit cliquer le lien dans l'email (hash #access_token=...) et reset via supabase.auth.updateUser côté client.
  return { status: "error", message: "Reset indisponible par ce canal — utilise le lien reçu par email." };
...
async function getCoursAdmin(p: Record<string, unknown>) {
  // Audit 2026-04-11 P1 : exiger is_admin sur le caller.
  const adminCode = String(p.code || "");
  const { data: admin } = await adminClient.from("profiles")
    .select("is_admin").eq("code", adminCode).maybeSingle();
  if (!admin?.is_admin) return { status: "error", message: "Accès refusé." };
...
async function saveCours(p: Record<string, unknown>) {
  // Audit 2026-04-11 P0 : exiger is_admin sur le caller.
  const adminCode = String(p.code || p.adminCode || "");
  const { data: admin } = await adminClient.from("profiles")
    .select("is_admin").eq("code", adminCode).maybeSingle();
  if (!admin?.is_admin) return { status: "error", message: "Accès refusé." };
...
async function cronSendEmails(_p: Record<string, unknown>) {
  // Audit 2026-04-11 P0 : gate par secret partagé.
  // Définir CRON_SECRET via `supabase secrets set CRON_SECRET=...` et l'inclure dans le payload Scheduler.
  const CRON_SECRET = Deno.env.get("CRON_SECRET") || "";
  const providedSecret = String(_p.cron_secret || "");
...
// ── send_admin_email — envoi custom Resend, gaté par compte admin ──
// Utilisé pour les mails one-shot personnalisés (premier contact, suivi VIP).
// Requiert adminCode d'un profil avec is_admin=true.

async function sendAdminEmail(p: Record<string, unknown>) {
  const adminCode = String(p.adminCode || "").trim().toUpperCase();
  ...
  if (!adminCode || !to || !subject || !html) {
    return { status: "error", message: "adminCode, to, subject, html requis." };
  }

  // Vérif admin
  const { data: prof } = await adminClient.from("profiles")
    .select("code, is_admin").eq("code", adminCode).limit(1);
  if (!prof || prof.length === 0 || !prof[0].is_admin) {
    return { status: "error", message: "Accès refusé." };
  }
...
const ACTIONS: Record<string, (p: Record<string, unknown>) => Promise<unknown>> = {
  ...
  // stripe_webhook volontairement hors ACTIONS map (audit 2026-04-11) —
  // accessible UNIQUEMENT via le path signature vérifiée en haut du dispatch (ligne ~1666).
  // Laisser ici permettait à n'importe qui de grante premium en envoyant {action:"stripe_webhook", email}.
